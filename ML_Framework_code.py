...
from state_manager import get_state_manager, create_status_dashboard
from cache_manager import (
    load_file_data, 
    create_cache_management_ui,
    performance_monitor,
    PerformanceMonitor
)
from optimized_functions import (
    perform_pca_optimized,
    perform_tsne_optimized,
    impute_missing_values_optimized,
    smooth_data_optimized,
    calculate_ndsi_optimized,
    evaluate_regression_models_optimized,
    display_processing_summary,
    create_operation_timeline
)
import functions  # Keep original functions as fallback
...
st.markdown("*Optimized with Advanced State Management & Performance Monitoring*")

# Add status dashboard if data is loaded
create_status_dashboard(state_manager)

# Optional: Cache management and performance dashboard in sidebar
with st.sidebar:
    st.markdown("---")
    with st.expander("⚙️ Performance & Cache"):
        create_cache_management_ui()
...
# Optimized data loading with state management and caching
if uploaded_file is not None:
    # Check if this is a new file
    current_file_info = state_manager.current_dataframe is not None and \
                       st.session_state.get('current_file_info', {}).get('filename') == uploaded_file.name
    ...
    if not current_file_info:
        try:
            with st.spinner("🔄 Loading and processing file..."):
                ...
                data = load_file_data(file_content, uploaded_file.name)
                
                # Load data into state manager
                state_manager.load_initial_data(data, uploaded_file.name)
    ...
    current_data = state_manager.current_dataframe
    if current_data is not None:
        st.subheader("📊 Current Dataset")
        
        # Show dataset with expandable view
        ...
        with col1:
            st.metric("Rows", current_data.shape[0])
        with col2:
            st.metric("Columns", current_data.shape[1])
        with col3:
            missing_count = current_data.isna().sum().sum()
        ...
        # NaN values analysis
        nan_counts = current_data.isna().sum()
        total_nans = nan_counts.sum()
        
        if total_nans > 0:
            st.warning(f"⚠️ Found {total_nans} missing values across {(nan_counts > 0).sum()} columns")
            with st.expander("View missing values by column"):
                missing_df = pd.DataFrame({
                    'Column': nan_counts.index,
                    'Missing Count': nan_counts.values,
                    'Missing %': (nan_counts.values / len(current_data) * 100).round(2)
                })
        ...
        with st.expander("📊 Data Types Overview"):
            dtype_summary = current_data.dtypes.value_counts()
            st.write("**Data Types Distribution:**")
            for dtype, count in dtype_summary.items():
                st.write(f"- {dtype}: {count} columns")
        
        # Show processing summary if available
        display_processing_summary(state_manager)
    
        # Option 1: Remove rows from the beginning
        st.subheader("🔧 Initial Data Preprocessing")
        ...
        if remove_rows:
            ...
            if st.button("🗑️ Apply Row Removal", key="remove_rows_btn"):
                try:
                    new_data = current_data.iloc[n_rows_to_remove:].reset_index(drop=True)
                    success = state_manager.update_dataframe(
                        new_data,
                        operation='remove_rows',
                        description=f'Removed first {n_rows_to_remove} rows',
                        metadata={'n_rows_removed': n_rows_to_remove}
                    )
                    
                    if success:
                        st.success(f"✅ Removed first {n_rows_to_remove} rows. New dataset size: {len(new_data)} rows")
                        st.rerun()
                    
                except Exception as e:
                    st.error(f"❌ Error removing rows: {str(e)}")

        # Feature columns selection
        st.header("Step 2: Feature Columns Selection")
        col1, col2 = st.columns([10, 1])
        with col1:
            # Get current features from state manager
            current_features = state_manager.get_features()
            available_columns = current_data.columns.tolist()
            
            # Use existing features or default to all columns
            default_features = current_features if current_features else available_columns
            
            features = st.multiselect(
                "Select feature columns", 
                available_columns, 
                default=default_features,
                help="Choose columns to use as input features for your model"
            )
        with col2:
        ...
        if has_categorical_columns:
            col1, col2 = st.columns([10, 1])
            with col1:
                # Get current categorical columns from state manager
                current_categorical = state_manager.get_categorical_columns()
                categorical_columns = st.multiselect(
                    "Select categorical columns", 
                    features, 
                    default=current_categorical,
                    help="Choose columns containing categorical data"
                )
            with col2:
        ...
        col1, col2 = st.columns([10, 1])
        with col1:
            target_options = [col for col in available_columns if col not in features]
            
            # Get current target from state manager
            current_target = state_manager.get_target_column()
            default_target_index = 0
            if current_target and current_target in target_options:
                default_target_index = target_options.index(current_target)
        ...
        # Apply button for feature + target selection
        if st.button("🎯 Apply Feature & Target Selection", key="apply_features_btn"):
            try:
                # Update metadata in state manager
                state_manager.update_feature_metadata(features, target_column, categorical_columns)
                
                # Filter data to selected columns
                selected_data = current_data[features + [target_column]]
                
                success = state_manager.update_dataframe(
                    selected_data,
                    operation='feature_target_selection',
                    description=f'Selected {len(features)} features and target column',
                    metadata={
                ...
                if success:
                    st.success(f"✅ Applied selection: {len(features)} features, target: {target_column}")
                    st.rerun()
                    
            except Exception as e:
                st.error(f"❌ Error applying selection: {str(e)}")
    ...
    if handle_missing:
        ...
        # Apply missing value handling with optimized function
        if st.button("🔧 Apply Missing Value Handling", key="apply_missing_values_btn"):
            method_map = {
            ...
            # Use optimized missing value imputation
            success = impute_missing_values_optimized(state_manager, method=method, k=k)
    ...
    if has_date_column:
        ...
        if st.button("🕒 Apply Time Resampling", key="apply_resampling_btn"):
            try:
                current_df = state_manager.current_dataframe
                ...
                temp_data = current_df.copy()
                temp_data[date_column] = smart_datetime_convert(temp_data[date_column])
                
                # Set date column as index for resampling
                temp_data = temp_data.set_index(date_column)
                
                # Apply resampling
                resampled_data = temp_data.resample(resample_frequency).agg(resample_method)
                resampled_data = resampled_data.reset_index()
                
                # Update state
                success = state_manager.update_dataframe(
                    resampled_data,
                    operation='time_resampling',
                    description=f'Applied {resample_method} resampling with {resample_frequency} frequency',
                    metadata={
                        'date_column': date_column,
                        'frequency': resample_frequency,
                        'method': resample_method,
                        'rows_before': len(current_df),
                        'rows_after': len(resampled_data)
                    }
                )
                ...
                with st.expander("Sample of date column data for debugging"):
                    ...
                    st.write(current_df[date_column].head(10).tolist())
        ...
        if apply_smoothing:
            ...
            if st.button("🌊 Apply Data Smoothing", key="apply_smoothing_btn"):
                ...
                success = smooth_data_optimized(state_manager, method=method, **smoothing_params)
                if success:
                    st.rerun()
